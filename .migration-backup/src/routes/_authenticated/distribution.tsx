...
import {
  Copy, ExternalLink, Loader2, Radio, RefreshCw, AlertTriangle, Sparkles,
} from "lucide-react";
import { fetchArticles } from "@/lib/articles-store";
import {
  fetchAllArticleAssets,
  fetchArticleAssets,
  updateArticleAsset,
  upsertArticleAssets,
  type ArticleAsset,
} from "@/lib/article-assets-store";
import {
  ARTICLE_CHANNELS,
  articleChannelLabel,
  type ArticleAssetStatus,
  type ArticleChannelId,
} from "@/lib/article-channels";
...
export const Route = createFileRoute("/_authenticated/distribution")({
  head: () => ({
    meta: [
      { title: "Distribution Center · AI Edge Solutions" },
      { name: "description", content: "Every channel and publication status across all articles." },
    ],
  }),
...
function DistributionPage() {
  ...
  const { data: allAssets = [] } = useQuery({
    queryKey: ["article_assets_all"],
    queryFn: fetchAllArticleAssets,
  });
      ...
      <div className="mx-auto max-w-6xl space-y-8">
        <div>
          ...
          <p className="mt-2 text-muted-foreground">
            ...
            Copy and paste them manually now — direct API publishing (Google Business Profile, Facebook Pages, Instagram Business, LinkedIn Company Pages, YouTube) is on the roadmap.
          </p>
        </div>
            ...
            <Card key={c.id}>
              <CardContent className="p-4">
                ...
                <div className="mt-2 flex flex-wrap gap-1 text-[10px]">
                  ...
                  {c.failed > 0 && (
                    <span className="rounded bg-red-100 px-1.5 py-0.5 text-red-900">F {c.failed}</span>
                  )}
                </div>
...
function ArticleAssetsPanel({ article }: { article: any }) {
  ...
  const generate = useServerFn(generateArticleAssets);
  ...
  const sorted = ARTICLE_CHANNELS.map((c) =>
    assets.find((a) => a.channel === c.id),
  );
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      await upsertArticleAssets(
        article.id,
        result.assets.map((a) => ({
          channel: a.channel as ArticleChannelId,
          body: a.body,
          status: a.status,
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" variant="outline" onClick={handleGenerate} disabled={busy || !article.body}>
          {busy ? <><Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> Generating…</> :
            assets.length === 0 ? <><Sparkles className="mr-1.5 h-3.5 w-3.5" /> Generate all assets</> :

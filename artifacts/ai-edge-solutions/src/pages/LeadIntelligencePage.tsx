...
type ConversionStage = "appointment_requested" | "scheduling" | "booked" | "completed" | "won" | "lost";

const CONVERSION_STAGES: ConversionStage[] = ["appointment_requested", "scheduling", "booked", "completed", "won", "lost"];
const CONVERSION_LABELS: Record<ConversionStage, string> = {
  appointment_requested: "Appointment requested", scheduling: "Scheduling", booked: "Booked",
...
const STAGE_COLORS: Record<string, string> = {
  appointment_requested: "#F59E0B", scheduling: "#A78BFA", booked: "#00AEEF",
  completed: "#22C55E", won: "#16A34A", lost: "#EF4444",
};
...
export default function LeadIntelligencePage() {
  ...
  const conversionMutation = useMutation({
    mutationFn: ({ id, stage }: { id: string; stage: ConversionStage }) =>
      authFetch(`/leads/${id}/conversion`, { method: "PATCH", body: JSON.stringify({ stage, note: conversionNote.trim() || undefined }) }),
    onSuccess: (_, vars) => { refresh(); setConversionNote(""); toast.success(`Lead marked ${CONVERSION_LABELS[vars.stage].toLowerCase()}`); },
    onError: () => toast.error("Could not update the conversion stage"),
  });
  ...
  const visible = useMemo(() => {
    ...
    if (CONVERSION_STAGES.includes(filter as ConversionStage)) return leads.filter(lead => lead.status === filter);
    return leads.filter(lead => lead.responseStatus === filter);
  }, [filter, leads]);
  const selected = leads.find(lead => lead.id === selectedId) ?? null;
  const selectLead = (lead: Lead) => { setSelectedId(lead.id); setDraft(lead.draftResponse ?? ""); setConversionNote(""); };
  ...
  const selectedStage = selected && CONVERSION_STAGES.includes(selected.status as ConversionStage) ? selected.status as ConversionStage : null;
  ...
  return <AppShell><div style={{ maxWidth: 1180, margin: "0 auto", fontFamily: "Inter, system-ui, sans-serif" }}>
    <div style={{ marginBottom: 24 }}>
      ...
      <h1 style={{ color: t.text, fontSize: 30, margin: 0 }}>Lead Pipeline</h1>
      <p style={{ color: t.text2, marginTop: 8, maxWidth: 780 }}>Review and send responses, track delivery and follow-up, then move each lead through appointment handoff and conversion.</p>
    </div>

    <div style={{ display: "grid", gridTemplateColumns: "repeat(5, minmax(0,1fr))", gap: 12, marginBottom: 20 }}>
      {[["Total leads", data?.stats.total ?? 0], ["Ready to send", leads.filter(l => l.responseStatus === "ready_for_review" || l.responseStatus === "approved").length], ["Follow-up due", data?.stats.followUpDue ?? 0], ["Booked", leads.filter(l => l.status === "booked").length], ["Won", leads.filter(l => l.status === "won").length]].map(([label, value]) =>
    ...
    <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
      {CONVERSION_STAGES.map(stage => <button key={stage} onClick={() => setFilter(stage)} style={{ border: `1px solid ${filter === stage ? STAGE_COLORS[stage] : t.border}`, background: filter === stage ? `${STAGE_COLORS[stage]}20` : "transparent", color: filter === stage ? STAGE_COLORS[stage] : t.text2, padding: "7px 12px", borderRadius: 999, cursor: "pointer", fontWeight: 700, fontSize: 12 }}>{CONVERSION_LABELS[stage]}</button>)}
    </div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: selected ? "minmax(0,1fr) 430px" : "1fr", gap: 16, alignItems: "start" }}>
      <div style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: 14, overflow: "hidden" }}>
        {isLoading ? <div style={{ padding: 28, color: t.text3 }}>Loading leads...</div> : visible.length === 0 ? <div style={{ padding: 36, color: t.text3, textAlign: "center" }}>No leads match this filter.</div> : visible.map(lead => {
          ...
          const stage = CONVERSION_STAGES.includes(lead.status as ConversionStage) ? lead.status as ConversionStage : null;
          return <button key={lead.id} onClick={() => selectLead(lead)} style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr 135px", gap: 16, width: "100%", textAlign: "left", padding: "16px 18px", border: 0, borderBottom: `1px solid ${t.border}`, borderLeft: active ? "3px solid #00AEEF" : lead.needsFollowUp ? "3px solid #F59E0B" : "3px solid transparent", background: active ? "rgba(0,174,239,.08)" : "transparent", cursor: "pointer" }}>
      ...
      {selected && <aside style={{ background: t.card, border: `1px solid ${t.border}`, borderRadius: 14, padding: 18, position: "sticky", top: 18 }}>
        ...
        {(selectedStage || selected.deliveryStatus || selected.needsFollowUp) && <div style={{ marginTop: 14, padding: 10, borderRadius: 9, background: selectedStage ? `${STAGE_COLORS[selectedStage]}18` : selected.needsFollowUp ? "rgba(245,158,11,.12)" : "rgba(0,174,239,.1)", color: selectedStage ? STAGE_COLORS[selectedStage] : selected.needsFollowUp ? "#F59E0B" : "#00AEEF", fontSize: 12, fontWeight: 800 }}>{selectedStage ? `Pipeline: ${CONVERSION_LABELS[selectedStage]}` : selected.needsFollowUp ? "Follow-up is due — no customer reply has been recorded." : `SMS status: ${selected.deliveryStatus?.replaceAll("_", " ")}`}</div>}
        ...
        <div style={{ marginTop: 18, paddingTop: 16, borderTop: `1px solid ${t.border}` }}>
          <div style={{ color: t.text3, fontSize: 11, fontWeight: 800, textTransform: "uppercase", marginBottom: 8 }}>Appointment and conversion</div>
          <input value={conversionNote} onChange={event => setConversionNote(event.target.value)} placeholder="Optional handoff or outcome note" style={{ width: "100%", boxSizing: "border-box", borderRadius: 9, border: `1px solid ${t.border}`, background: "rgba(0,0,0,.18)", color: t.text, padding: 10, marginBottom: 9 }} />
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7 }}>
            {CONVERSION_STAGES.map(stage => <button key={stage} onClick={() => conversionMutation.mutate({ id: selected.id, stage })} disabled={conversionMutation.isPending || selected.status === stage} style={{ padding: 9, borderRadius: 8, border: `1px solid ${STAGE_COLORS[stage]}55`, background: selected.status === stage ? `${STAGE_COLORS[stage]}28` : `${STAGE_COLORS[stage]}10`, color: STAGE_COLORS[stage], cursor: "pointer", fontSize: 11, fontWeight: 800 }}>{CONVERSION_LABELS[stage]}</button>)}
          </div>
          <div style={{ color: t.text3, fontSize: 11, lineHeight: 1.5, marginTop: 9 }}>Booking and closed stages automatically suppress follow-up. This records the handoff; it does not promise or create an appointment by itself.</div>
        </div>
        ...
        <div style={{ marginTop: 18 }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 7 }}><div style={{ color: t.text3, fontSize: 11, fontWeight: 800, textTransform: "uppercase" }}>Proposed response</div><button onClick={() => analyzeMutation.mutate(selected.id)} disabled={analyzeMutation.isPending || ["sending", "sent", "replied", "opted_out"].includes(selected.responseStatus)} style={{ border: "1px solid rgba(0,174,239,.35)", background: "rgba(0,174,239,.1)", color: "#00AEEF", borderRadius: 8, padding: "6px 9px", cursor: "pointer", fontSize: 11, fontWeight: 800 }}>{selected.draftResponse ? "Re-analyze" : "Analyze"}</button></div>
          <textarea value={draft} onChange={event => setDraft(event.target.value)} rows={6} disabled={["sending", "sent", "replied", "opted_out"].includes(selected.responseStatus)} placeholder="Analyze this lead to generate a review draft." style={{ width: "100%", boxSizing: "border-box", resize: "vertical", borderRadius: 10, border: `1px solid ${t.border}`, background: "rgba(0,0,0,.18)", color: t.text, padding: 12, lineHeight: 1.5 }} />
        </div>

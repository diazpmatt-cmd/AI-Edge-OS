...
describe("manual lead analysis route", () => {
  it("requires authentication before invoking analysis", async () => {
    ...
    const handler = createLeadAnalysisHandler(analyze as any, unauthenticated);
  ...
  it("returns a review draft without sending it", async () => {
    const analyze = vi.fn().mockResolvedValue({
      ...
      analysis: {
        ...
        summary: "Customer reports bed bugs on a couch in Daphne.",
        missingInformation: ["What is the best time to contact you?"],
        draftResponse: lead.draftResponse,
      },
    });
    const response = makeResponse();
    const handler = createLeadAnalysisHandler(analyze as any, authenticated);
    ...
    expect(response.body).not.toHaveProperty("dispatched");
  });
  ...
  ] as const)("maps %s to HTTP %s", async (error, expectedStatus) => {

...
function authorizationRef(approval: ApprovalRecord): string {
...
function assertApproval(input: {
  ...
  const { approval, category, mission, now } = input;
  if (approval.taskId !== mission.taskId || approval.decision !== "approved" || !approval.categories.includes(category)) throw new Error(`DAB_GIT_MISSION_${category.toUpperCase()}_AUTHORIZATION_INVALID`);
  if (approval.expectedGitSha !== mission.expectedBaseSha || approval.specificationHash.length < 6 || !approval.approvalId.startsWith("approval_")) throw new Error(`DAB_GIT_MISSION_${category.toUpperCase()}_AUTHORIZATION_STALE`);
  if (approval.expiresAt && new Date(approval.expiresAt).getTime() <= now.getTime()) throw new Error(`DAB_GIT_MISSION_${category.toUpperCase()}_AUTHORIZATION_EXPIRED`);
}

export function assertDabGitMissionAuthorizations(mission: DabResolvedGitMission, now = new Date()): void {
  for (const category of DAB_GIT_MUTATION_AUTHORIZATIONS) assertApproval({ approval: mission.approvals[category], category, mission, now });
}
...
export class DabGitMissionRunner {
  ...
  private binding() {
    ...
    return buildDabGitApplyBinding({
      ...
      preparationJobId: m.preparationJobId,
      proposalId: m.proposalId,
      proposalFingerprint: m.proposalFingerprint,
      capability: "prepare_code_patch",
      contextHash: m.contextHash,
      manifestContent: m.manifestContent,
      manifestSha256: m.manifestSha256,
      expectedBaseSha: m.expectedBaseSha,
      branchName: m.branchName,
      editingAuthorizationRef: authorizationRef(m.approvals.editing),
    });
  }
  ...
  private async rehydrateApply(binding: ReturnType<DabGitMissionRunner["binding"]>): Promise<void> {
    const applied = await this.input.workspace.applyExactFiles({
  ...
  async runOne(): Promise<DabGitMissionResult> {
    ...
    const pushFingerprintMaterial = { repositoryId: commitReceipt.repositoryId, branchName: commitReceipt.branchName, commitSha: commitReceipt.commitSha, expectedRemoteSha: null, pushAuthorizationRef: authorizationRef(this.input.mission.approvals.pushing) };
    const pushKey = `dab-git-push:${await this.syntheticFingerprint(pushFingerprintMaterial)}`;
    ...
    const prFingerprintMaterial = { repositoryId: pushReceipt.repositoryId, headBranch: pushReceipt.branchName, headSha: pushReceipt.commitSha, baseBranch: "main", baseSha: binding.expectedBaseSha, prAuthorizationRef: authorizationRef(this.input.mission.approvals.pull_request_creation) };
    const prKey = `dab-git-pr:${await this.syntheticFingerprint(prFingerprintMaterial)}`;
    ...
    if (ciReceipt.outcome === "pending") return this.result("waiting_ci", "ci", { applyReceipt, commitReceipt, pushReceipt, prReceipt, ciReceipt });
    if (ciReceipt.outcome !== "green") throw new Error(`DAB_GIT_MISSION_CI_${ciReceipt.outcome.toUpperCase()}`);

    const mergeFingerprintMaterial = { repositoryId: prReceipt.repositoryId, prNumber: prReceipt.prNumber, headSha: prReceipt.headSha, baseSha: prReceipt.baseSha, mergeMethod: "squash", mergeAuthorizationRef: authorizationRef(this.input.mission.approvals.merging), ciEvidenceDigest: ciReceipt.evidenceDigest };
    const mergeKey = `dab-git-merge:${await this.syntheticFingerprint(mergeFingerprintMaterial)}`;
  ...
  private async syntheticFingerprint(value: unknown): Promise<string> {

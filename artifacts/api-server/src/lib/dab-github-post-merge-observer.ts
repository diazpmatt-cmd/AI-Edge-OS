...
import { DAB_GIT_ALLOWED_REPOSITORY_ID, type DabGitWorkerConfig } from "./dab-git-worker-config.js";
...
export class DabGitHubPostMergeObserver implements DabGitPostMergeObserver {
  constructor(private readonly input: { config: DabGitWorkerConfig; http: DabGitHubHttpClient }) {}
  ...
  async observe(input: { repositoryId: string; mergeSha: string; files: readonly { path: string; sha256: string }[] }) {
    ...
    const main = await this.input.http.request<{ object?: { sha?: string } }>({ method: "GET", path: "/git/ref/heads/main" });
    ...
    const commit = await this.input.http.request<{ sha?: string }>({ method: "GET", path: `/commits/${input.mergeSha}` });
    ...
    for (const expected of input.files) {
      ...
      const file = await this.input.http.request<{ type?: string; encoding?: string; content?: string; size?: number }>({ method: "GET", path: `/contents/${encodeContentPath(expected.path)}?ref=${input.mergeSha}` });

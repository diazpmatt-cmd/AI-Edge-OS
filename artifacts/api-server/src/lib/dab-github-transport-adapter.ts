...
function assertMain(branch: string): void {
  if (branch !== "main") throw new Error("DAB_GITHUB_TRANSPORT_BASE_BRANCH_INVALID");
}

function encodeRef(branch: string): string {
  assertDabGitBranchName(branch);
  return encodeURIComponent(`refs/heads/${branch}`);
}
  ...
  return {
    async request<T>({ method, path, body }) {
      ...
      if (!path.startsWith("/") || path.includes("..") || path.includes("\\") || path.includes("://")) {
        throw new Error("DAB_GITHUB_TRANSPORT_PATH_INVALID");
      }
      ...
      if (text) {
        ...
        catch { throw new Error("DAB_GITHUB_TRANSPORT_RESPONSE_INVALID"); }
      }
...
export class DabGitHubTransportAdapter implements DabGitPushPrAdapter, DabGitMergeAdapter {
  ...
  constructor(options: DabGitHubTransportOptions) {
    this.config = options.config;
    this.git = options.git;
    this.http = options.http ?? realHttp(options.config);
  }
  ...
  async pushCommit(input: { repositoryId: string; branchName: string; commitSha: string; expectedRemoteSha: string | null; idempotencyKey: string }): Promise<{ branchName: string; headSha: string }> {
    ...
    assertRepository(input.repositoryId);
    assertDabGitBranchName(input.branchName);
    assertSha(input.commitSha, "DAB_GITHUB_TRANSPORT_COMMIT_SHA_INVALID");
    ...
    const lease = input.expectedRemoteSha === null
      ? `--force-with-lease=refs/heads/${input.branchName}:`
      : `--force-with-lease=refs/heads/${input.branchName}:${input.expectedRemoteSha}`;
  ...
  async createPullRequest(input: { repositoryId: string; headBranch: string; headSha: string; baseBranch: string; baseSha: string; idempotencyKey: string }) {
    this.assertReady();
    assertRepository(input.repositoryId);
    assertDabGitBranchName(input.headBranch);
    assertMain(input.baseBranch);
    assertSha(input.headSha, "DAB_GITHUB_TRANSPORT_HEAD_SHA_INVALID");
    assertSha(input.baseSha, "DAB_GITHUB_TRANSPORT_BASE_SHA_INVALID");
    ...
    const response = await this.http.request<any>({
      ...
      path: "/pulls",
      body: {
        title: `Apollos authorized change ${input.idempotencyKey.slice(-12)}`,
        head: input.headBranch,
        base: "main",
        body: "Bounded DAB engineering change. Scope, authorization, and receipts are recorded in the canonical Development Control ledger.",
        maintainer_can_modify: false,
      },
    });
    if (response.status !== 201 || !Number.isInteger(response.body?.number) || response.body.number <= 0 || response.body?.head?.ref !== input.headBranch || response.body?.head?.sha !== input.headSha || response.body?.base?.ref !== "main" || response.body?.base?.sha !== input.baseSha) {
      throw new Error("DAB_GITHUB_TRANSPORT_PR_VERIFICATION_FAILED");
    }
  ...
  async mergeExact(input: { repositoryId: string; prNumber: number; expectedHeadSha: string; mergeMethod: "squash" | "merge" | "rebase"; idempotencyKey: string }) {
    this.assertReady();
    assertRepository(input.repositoryId);
    assertSha(input.expectedHeadSha, "DAB_GITHUB_TRANSPORT_HEAD_SHA_INVALID");

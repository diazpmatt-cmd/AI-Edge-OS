...
export function createGmailFetch(input: {
  ...
  return async (path, accessToken) => {
    if (!path.startsWith("/messages")) throw new Error("Gmail client is restricted to message list/get paths");
...
export async function listGmailMessageIds(input: {
  ...
  if (!Number.isInteger(input.maxPages) || input.maxPages < 1) throw new Error("maxPages must be positive");
  const query = input.query.trim();
  if (!query || query.length > MAX_GMAIL_QUERY_CHARS) throw new Error("Gmail query is missing or too long");
  const ids: string[] = [];
  ...
  for (let page = 0; page < input.maxPages; page += 1) {
    ...
    const result = await input.gmailFetch(`/messages?${params}`, input.accessToken);

...
export function buildRecoveryEvidence(events: readonly CustomerJourneyEvent[], clientId: string): RecoveryEvidence {
  ...
  const texts = scoped.filter(event => event.eventType === "recovery_text_sent" && event.canonicalRecordType === "telnyx_message" && event.canonicalRecordId && missedIds.has(metadataId(event, "parentCallId") ?? ""));

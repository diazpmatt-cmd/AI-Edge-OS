 ...
 * project slug. In that form the slug is resolved to the canonical client ID
 * before lead data is queried; there is no global or BB&B fallback.
 */
export async function computeTelnyxAnalytics(
  ...
  if (!resolvedProjectId) {
    ...
    if (!client) throw new Error("Analytics client not found");
    clientId = client.id;
  ...
  const eventRows = await db
    ...
    .from(leadsTable)
    .where(and(eq(leadsTable.clientId, clientId), realRowsFilter()))
  ...
  const recoveryRate = missedCalls > 0 ? Math.round((recoveredLeads / missedCalls) * 100) : null;
  ...
  const estimatedMissedRevenueCents = avgTicketCents !== null && missedCalls > 0
    ...
    : null;
  const avgTicketFmt = avgTicketCents !== null ? centsToDisplay(avgTicketCents) : null;
  ...
  return {
    ...
    estimated_missed_revenue_fmt: estimatedMissedRevenueCents !== null ? centsToDisplay(estimatedMissedRevenueCents) : null,

...
const safeDraftSchema = z.string().trim().min(1).max(800).superRefine((value, context) => {
  if (unsafeDraftLanguage.some((pattern) => pattern.test(value))) {
    context.addIssue({
      code: "custom",
      message: "Draft contains prohibited or unverified language",
    });
  }
});
...
export interface LeadReviewRepository {
  ...
  saveReview(
    leadId: string,
    update: Pick<Lead, "draftResponse" | "responseStatus">,
  ): Promise<Lead | null>;
}
...
export function createDrizzleLeadReviewRepository(
  ...
  return {
    ...
    async saveReview(leadId, update) {
      const [lead] = await database
        .update(leadsTable)
...
export class LeadReviewService {
  constructor(
    private readonly repository: LeadReviewRepository = createDrizzleLeadReviewRepository(),
  ) {}

  async reviewLead(leadId: string, input: unknown): Promise<LeadReviewResult> {
    ...
    if (parsed.data.action === "approve") {
      if (lead.responseStatus !== "ready_for_review" || !lead.draftResponse?.trim()) {
        return { status: "invalid", error: "draft_not_ready" };
      }
      const updated = await this.repository.saveReview(leadId, {
        draftResponse: lead.draftResponse,
        responseStatus: "approved",
      });
      return updated
        ? { status: "approved", lead: updated }
        : { status: "failed", error: "persistence_failure" };
    }

    if (parsed.data.action === "edit") {
      if (!["ready_for_review", "approved"].includes(lead.responseStatus)) {
        return { status: "invalid", error: "draft_not_ready" };
      }
      const updated = await this.repository.saveReview(leadId, {
...
export async function reviewLead(leadId: string, input: unknown): Promise<LeadReviewResult> {
  return defaultLeadReviewService.reviewLead(leadId, input);
}
